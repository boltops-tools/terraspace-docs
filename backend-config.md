...
Notice, the variable notation available. Terraspace treats `key` and `prefix` values specially and expands them out, substituting actual values. The variables available are covered below. The generated default `key` string accounts for switching `account`, `region`, `env`, etc. Here's an expanded example:

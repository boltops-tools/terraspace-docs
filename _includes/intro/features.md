...
* [Tfvars]({% link _docs/tfvars.md %}): Use the same code with different tfvars to create multiple environments. Terraspace conventionally loads tfvars from the `tfvars` folder.
* [Layering]({% link _docs/tfvars/layering.md %}): Rich layering support. This allows you to build different environments like dev and prod with the same code.
